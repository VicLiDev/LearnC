/*
 * 要尝试这个例子,应在一个窗口中运行服务端程序。指定一个套接字文件的路径
 * ——例如 /tmp/socket 作为参数:
...
/*
 * 不断从套接字读取并输出文本信息直到套接字关闭。当客户端发送"quit"消息的时候返回
 * 非 0 值,否则返回 0。
 */
...
int main(int argc, char* const argv[])
{
    ...
    ret = bind(socket_fd, (struct sockaddr *)&name, SUN_LEN(&name));
    /* 监听连接。*/
    listen(socket_fd, 5);
