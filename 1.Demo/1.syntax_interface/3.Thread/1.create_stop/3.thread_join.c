/*
 * 有时候需要让一个线程去执行一段代码，我们需要知道它是否帮我们完成了指定的要求，
 * 或者异常终止，这时候我们就需要获取线程运行结果，线程退出可以通过返回值带出或者
 * 通过pthread_exit()参数带出，拿到它的“遗言”。
 *
 * 还有一个用处，在上面的例子中，我们都是在主线程中sleep()函数来防止新创建的线程
 * 还未结束，整个进程就结束，而现在我们可以用线程等待函数来达到这个目的。
 *
 * #include <pthread.h>
 ...
 *  函数调用出错时会返回非零错误码。
 *
 * 默认情况下线程是可以join的。但是，线程也可以detach（分离），使得线程不可join。
 * 可以join的线程在被join之前占有的系统资源不会被释放。而detach的进程会在进程结束
 * 的时候自动释放资源。
 */
