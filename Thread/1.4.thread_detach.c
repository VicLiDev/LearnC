...
// 当线程属于可结合时，它需要被其他线程join或者cancel回收资源,同时可以获取改进成的返回信息。相反一个已经处于分离的线程是不能被join或cancel的，
...
// 
// 其实在上面的例子中，已经有过通过join将一个线程分离，但是当在一个线程中通过调用pthread_join()来回收资源时，调用者就会被阻塞，如果需要回收的
