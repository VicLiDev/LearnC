...
（物理内存大小）、Pss（按比例的共享内存）、Private_Dirty（私有脏页）等，帮助你
简化和理解进程的内存使用。
...
* filename: 被加载动态库的路径，如果这个路径是绝对路径，则该函数将会尝试直接打开
            ...
            库文件：
            - 查找有环境变量LD_LIBRARY_PATH指定的一系列目录
...
对于操作系统里数量庞大的共享对象，操作系统一般会对共享对象的目录组织和使用方法有
一定的规则。
...
* 次版本号：增量升级，高的次版本号的库向后兼容低的次版本号的库
* 发布版本号：表示库的一些错误的修正、性能的改进等，不同的发布版本号之间完全兼容

2. SO-NAME

对于新的系统来说，例如Solaris和Linux，普遍采用SO-NAME的命名机制来记录共享库的依赖
关系。每个共享库都有一个对应的“SO-NAME”，这个SO-NAME即共享库的文件名去掉次版本号
和发布版本号，只保留主版本号。例如：libfoo.so.2.6.1的SO-NAME为libfoo.so.2。

“SO-NAME”相同的两个共享库，次版本号大的兼容次版本号小的。在Linux系统中，系统会为
每个共享库在他所在的目录创建一个跟“SO-NAME”相同的并且指向他的软链接。

ldconfig：当系统中安装或更新一个共享库时，运行这个工具，它会遍历所有的默认共享目录，
比如/lib、/usr/lib等，然后更新/创建所有的软链接，使他们指向最新版的共享库。

可以通过如下方法查看SO-NAME：
```
readelf -d libmlib.so | grep SONAME
objdump -x libmlib.so | grep SONAME
```

### 符号版本

次版本号交会问题：app编译时使用了次版本号新的库，运行时系统中只有次版本号旧的库，
这时候app可能使用了新版本库的接口，也可能没使用，有两种处理策略
1. 通过警告告知用户，程序正常执行
2. 考虑到接口兼容问题，不执行程序

涉及到SO-NAME时，只要app需要的SO-NAME和系统中存在的库的SO-NAME一致，就认为接口兼容。
但这并不能解决问题，次版本号交会的问题依旧存在。因此就有了符号版本机制来解决这个问题。

**基于符号的版本机制**

基于符号的版本机制（Symbol Versioning）的方案的基本思路是让每个导出和导入的符号
都有一个相关联的版本号，它的实际做法类似于名称修饰的方法

**Solaris中的符号版本机制**

Solaris 的 ld 链接器为共享库新增了版本机制（Versioning）和范围机制（Scoping）。

在Solaris中，程序员可以在链接共享库时编写一种叫做符号版本脚本的文件，在这个文件中
指定这些符号与集合之间的继承依赖关系。链接器在链接时根据符号版本脚本中指定的关系来
产生共享库，并设置符号的集合和他们之间的关系。

在程序运行时，动态链接器会通过程序内记录的他所依赖的所有共享库的符号集合版本信息，
然后判定当前系统共享库中的符号集合版本是否满足运行要求。

**Linux中的符号版本**

Linux系统下共享库的符号版本机制并没有被广泛应用，主要使用共享库符号版本机制的是Glibc
软件包中所提供的20多个共享库。

GCC在Solaris系统中的符号版本机制的基础上还提供了两个扩展。第一个扩展是，除了可以在
符号版本脚本中指定符号的版本之外，GCC还允许使用一个叫做“.symver”的汇编宏指令来指定
符号版本。第二个扩展是，GCC允许多个版本的同一个符号存在于一个共享库中。

Solaris2.5系统有一个不足，在同一个共享库中，每个函数只能有一个版本号。Linux下的符号
版本机制比Solaris2.5的要先进一些，它允许同一个名称的符号存在多个版本。

### 共享库系统路径

目前大多数包括Linux在内的开源操作系统都遵循一个叫做FHS（File Hierarchy Standard）的
标准，这个标准规定了一个系统中的系统文件应该如何存放，包括各个目录的结构、组织和作用，
这有利于促进各个开源操作系统之间的兼容性。共享库的存放方式也被FHS列入了规定范围。一个
系统中主要有3个存放共享库的位置：
* /lib， 主要存放最关键、最基础的共享库，例如动态链接器、C库，这些库主要是那些/bin
  和/sbin下的程序所要用到的库，还有系统启动时需要的库
* /usr/lib，主要是非系统运行时所需要的关键性的共享库，主要是一些开发时用到的共享库
* /usr/local/lib，放置跟操作系统本身并不相关的库，主要是一些第三方的应用程序的库。
  与之相对应的可执行程序可能被放到/usr/local/bin下。GNU的标准推荐第三方的程序应该
  默认将库安装到/usr/local/lib下

### 共享库的查找过程

动态链接库所依赖的模块路径保存在“.dynamic”段里面，由DT_NEED类型的项表示。动态链接器
对于依赖模块的查找有一定的规则：
* 如果DT_NEED里面保存的是绝对路径，那么动态链接器就按照找这个路径去查找
* 如果DT_NEED里面保存的是相对路径，那么动态链接器会在/lib、/usr/lib和由/etc/ld.so.conf
  配置文件指定的目录中查找共享库。为了程序的可移植性和兼容性，共享库的路径往往是相对的。

ld.so.conf 是一个文本配置文件，它可能包含其他的配置文件，这些配置文件中存放着目录
信息，例如：
```
$ cat /etc/ld.so.conf
...
如果动态链接器在每次查找共享库时都去遍历这些目录，那将会非常耗时。所以系统中都有一个
叫做ldconfig的程序，这个程序的作用是为共享库目录下的各个共享库创建、删除或更新相应
的SO-NAME（即相应的符号链接），这样每个共享库的SO-NAME就能够指向正确的共享库文件；
并且这个程序还会将这些SO-NAME收集起来，集中存放到/etc/ld.so.cache文件里，并建立一个
SO-NAME的缓存。当动态链接器要查找共享库时，它可以直接从/etc/ld.so.cache里面查找。并
建立一个SO-NAME的缓存。当动态链接器要查找共享库时，它可以直接从/etc/ld.so.cache里面
查找。而/etc/ld.so.cache的结构是经过特殊设计的，非常适合查找，所以这个设计大大加快了
共享库的查找过程。

如果动态链接器在/etc/ld.so.cache里面没有找到需要的共享库，那么还会遍历/lib和/usr/lib
这两个目录，如果还是没找到，就宣告失败。

很多软件包的安装程序在往系统中安装共享库以后都会调用ldconfig

### 环境变量

* **LD_LIBRARY_PATH**

改变共享库查找路径最简单的方法是使用LD_LIBRARY_PATH环境变量，这个方法可以临时改变
某个应用程序的共享库查找路径，而不会影响系统中的其他程序。默认情况下LD_LIBRARY_PATH
为空。这个环境变量可以很方便的让我们测试新的共享库或使用非标准的共享库。

Linux中还有一种方法可以实现与LD_LIBRARY_PATH类似的功能，那就是直接运行动态链接器来
启动程序，例如：
```
/lib64/ld-linux-x86-64.so.2 --library-path `pwd` ./main
```
注意这里的`/lib64/ld-linux-x86-64.so.2`和系统中的`ld`是不同的，系统中的`ld`主要是
在编译过程中进行链接的作用，而`/lib64/ld-linux-x86-64.so.2`是在运行时作为动态链接器
起作用。可以使用ldd命令查看动态链接器的位置

动态链接器会按照如下顺序依次装载或查找共享对象（目标文件）：
* 由环境变量LD_LIBRARY_PATH指定的路径
* 由路径缓存文件/etc/ld.so.cache指定的路径
* 默认共享库目录，先/usr/lib，然后/lib

LD_LIBRARY_PATH对于共享库的开发和测试来说十分方便，但是他不应该被滥用。普通用户在
正常情况下不应该随意设置LD_LIBRARY_PATH来调整共享库搜索目录。

* **LD_PRELOAD**

LD_PRELOAD 可以指定预先装载的一些共享库或是目标文件。它比LD_LIBRARY_PATH里面指定
的目录中的共享库还要优先。无论程序是否依赖它们，LD_PRELOAD里面指定的共享库或目标
文件都会被装载。

由于全局符号介入这个机制的存在，LD_PRELOAD里面指定的共享库或目标文件中的全局符号
就会覆盖后边加载的同名全局符号，这是的我们可以很方便的做到改写标准C库中的某个或者
某几个函数而不影响其他函数，这对于程序的调试或者测试非常有用。同样，正常情况下应该
尽量避免使用LD_PRELOAD。

系统配置文件中有一个文件是/etc/ld.so.preload，它的作用与LD_PRELOAD一样。这个文件
里面记录的共享库或目标文件的效果跟LD_PRELOAD里面指定的一样，也会被提前装载。

* **LD_DEBUG**

LD_DEBUG可以打开动态链接器的调试功能，当设置这个变量时，动态链接器会在运行时打印出
各种有用的信息，对于我们开发和调试共享库有很大的帮助。例如可以将LD_DEBUG设置成“files”，
并且运行一个简单动态链接的HelloWorld：
```
LD_DEBUG=files ./HelloWorld.out
```

LD_DEBUG可以设置的值如下：
* files: 打印整个装载过程，显示app依赖于哪个共享库并且按照什么步骤装载和初始化，
  共享库装载时地址等
...
创建共享库，最关键的是使用GCC的两个参数：
* -shared:  表示输出结果是共享库类型
* -fPIC:    表示使用地址无关代码技术来生成输出文件

另外还有一个参数是`-W`，这个参数可以将指定的参数传递给链接器，用来指定输出共享库
的SO-NAME。例如：
```
$gcc -shared -Wl,-soname,my_soname -o library_name source_files library_files
```

如果不使用 -soname 来指定共享库的 SO-NAME，那么该共享库默认就没有SO-NAME，即使用
ldconfig更新SO-NAME的软链接时，对该共享对象也没有效果。

类似LD_LIBRARY_PATH，可以使用链接器的“-rpath”选项（或者GCC的-Wl,-rpath），这种方法
可以指定链接产生的目标程序的共享库查找路径。例如：
```
$ld -ppath /home/mylib -o program.out program.o -lsomelib
```

* **清除符号信息**

strip 工具清除掉共享库或可执行文件的所有符号和调试信息（“strip”是binutils的一部分）：
```
strip libfoo.so
```

去除符号和调试信息以后的文件往往比之前要小很多，一般只有原来的一半大小，甚至不到一半，
除了使用“strip”，还可以使用ld的`-s`和`-S`参数，使得链接器生成输出文件时就不产生符号
信息。“-s”和“-S”的区别是：“-S”消除调试符号信息，而`-s`消除所有符号信息。也可以在gcc中
通过`-Wl-s`和`-Wl,-S`给ld传递着两个参数。


* **共享库的安装**

很多时候没有系统的root权限，自然就不能执行ldconfig命令，当然也可以通过其他方法安装，
共享库安装无非是建立相应的SO—NAME软链接，并告诉编译器和程序如何查找该共享等，建立
SO-NAME的办法也是使用ldconfig，只不过需要指定共享库所在的目录：
```
$ldconfig -n shared_library_directory
```
-n 选项意味着“只更新缓存，不添加新的库到缓存中”。这个选项使得 ldconfig 不会扫描
目录来添加新库，而只是更新已经记录在缓存中的库的链接。

* **共享库构造和析构函数**

GCC提供了一种共享库的构造函数，只要在函数声明时加上`__attribute__((constructor))`
的属性，即指定该函数为共享库构造函数，拥有这种属性的函数会在共享库加载时被执行，
即在程序的main函数之前执行。如果使用dlopen()打开共享库，共享库构造函数会在dlopen()
返回之前被执行。

与共享库构造函数相对应的是析构函数：`__attribute__((destructor))`，析构函数会在
main()函数执行完毕之后执行（或者是程序调用exit()时执行）。如果共享库是运行时加载
的，那么使用dlclose()卸载共享库时，析构函数将会在dlclose()返回之前执行。
...
如果使用了这种析构或构造函数，那么必须使用系统默认的标准运行库和启动文件，即不可以
使用GCC的`-nostartfiles`或`-nostdlib`这两个参数。因为这些构造和析构函数是在系统默认
的标准运行库和启动文件里面被运行的，如果没有这些辅助结构，构造和析构函数不会被执行。

如果有多个构造和析构函数，执行顺序是没有规定的，是随机执行的。如果希望构造和析构
函数能够按照一定的顺序执行，GCC为提供了优先级机制。可以指定某个构造或者析构函数的
优先级：
```
void __attribute__((constructor(5))) init_function(void);
void __attribute__((destructor(10))) fini_function(void);
```
对于构造函数，优先级数字越小优先级越高，而对于析构函数，刚好相反，这是为了便于构造
和析构函数匹配，特别是涉及到资源申请释放的时候。


## 内存管理

Linux进程地址空间布局（ver：2.4.x）
```
+-------------------+ 0xffffffff
|    kernel space   |
+-------------------+ 0xc0000000
...
函数调用时，存在多种调用惯例，默认的调用惯例时cdecl。如果没有显示指定调用惯例，
则默认是cdecl惯例，其形式如下：
```
int _cdec1 foo(int n, float m)
```
其中`_cdecl`是非标准关键字，在不同的编译器里可能有不同的写法，例如在gcc里就不存在
`_cdecl`，而是使用`__attribute__((cdecl))`

foo函数调用过程分析：
1. 将m压入栈
2. 将n压入栈
3. 调用_foo（foo修饰之后为_foo）
    * 将返回地址（即调用_foo之后的下一条指令的地址）压入栈
    * 跳转到_foo执行
4. 保存现场
5. 实现功能
6. 恢复现场
7. 当函数返回之后，sp = sp + 8 （参数出栈）
...
| 调用惯例 | 出栈方 | 参数传递 | 名字修饰 |
|--|--|--|--|
| cdecl    | 函数调用方 | 从左到右的顺序压参数入栈 | 下划线+函数名                              |
| stdcall  | 函数本身   | 从右到左的顺序压参数入栈 | 下划线+函数名+@+参数的字节数，如：_func@12 |
| fastcall | 函数本身   | 前两个DOWRD(4byte)类型或者占更少字节的参数被放入寄存器，剩下的参数按照从右到左的顺序压栈 | @+函数名+@+参数的字节数 |
| pascal   | 函数本身   | 从左到右的顺序压参数入栈 | 较为复杂，参考pascal文档 |

不少编译器还提供一种称为naked call的调用惯例，这种调用惯例用在特殊的场合，其特点是
编译器不产生任何保护寄存器的代码，故称为naked call。

C++有一种特殊的调用惯例，称为this call，专用于类成员函数的调用


eax是传递返回值的通道，函数将返回值存储在eax中，返回后函数的调用方再读取eax，eax有
4byte，对于5-8字节对象的情况几乎所有的调用惯例都是采用eax和edx联合返回的方式进行，
eax存储低4字节，edx存储高1-4字节。

对于超过8字节的返回类型：
1. 在主调方栈上额外开辟了一片空间，并将这块空间的一部分作为传递返回值的临时对象，
   这里称为temp
2. 将temp对象的地址作为隐藏参数传递给被调函数
3. 被调函数将数据拷贝给temp对象，并将temp对象的地址用eax传出
4. 被调函数返回后，主调函数将eax指向的temp对象的内容拷贝到当前栈帧接收返回值的变量

### 堆-进程堆管理

进程的地址空间中，除了可执行文件、共享库和栈之外，剩余的未分配的空间都可以被用来
作为堆空间。Linux下的进程堆管理提供了两种堆空间分配的方式，即两个系统调用：
1. brk()系统调用
2. mmap()系统调用

brk()的作用实际上就是设置进程数据段的结束地址，即他可以扩大或者缩小数据段（Linux
下数据段和BSS合并在一起统称为数据段）。如果我们将数据段的结束地址向高地址移动，那么
扩大的那部分空间就可以被我们使用。

Glibc中还有一个函数叫做sbrk()，它实际上是brk系统调用的封装，底层是通过brk实现的。

mmap()的作用就是向操作系统申请一段虚拟地址空间，当然这块虚拟地址空间可以映射到某个
文件（这是这个系统调用最初的作用），当他不将地址空间映射到某个文件时，又称这块空间
为匿名空间，匿名空间就可以拿来作为堆空间。

glibc的malloc函数，对于小于128KB的请求来说，他会在现有的对空间里，按照堆分配算法
为它分配一块空间并返回；对于大雨128KB的请求来说，他会使用mmap()函数为它分配一块
匿名空间，然后在这个匿名空间中为用户分配空间，需要注意mmap()函数申请的空间的起始
地址和大小都必须是系统页的大小的整数倍。

在Linux2.6版本里，共享库的装载地址已经挪到了靠近栈的位置，即位于0xbfxxxxxx附近，
到进程，所以从0xbfxxxxxx到进程用brk()设置的边界末尾，没有任何空间占用的情况，都
可以作为一整块堆空间来使用。
...
栈都是“向下”增长，这是出于历史和效率的考虑：
* 栈从高地址向低地址增长（向下），而堆（heap）通常从低地址向高地址增长（向上）
* 可以让栈和堆在同一地址空间中向相反方向增长，更高效地使用内存
* 栈操作（push/pop）只需对栈指针做 `sub`/`add` 操作
...
| 架构     | 栈指针   | 栈增长方向    | 示例                        |
| -------- | -------- | ------------- | --------------------------- |
| x86      | ESP      | 向下（高→低） | `push eax` → `ESP -= 4`     |
| x86-64   | RSP      | 向下（高→低） | `push rax` → `RSP -= 8`     |
| ARM (32) | SP (R13) | 向下（高→低） | `push {r0}` → `SP -= 4`     |
| ARM64    | SP       | 向下（高→低） | `stp x29, x30, [sp, #-16]!` |
...
```asm
push rax      ; RSP -= 8
call func     ; return addr pushed at RSP - 8
```
...
ARM64（AArch64）：
```asm
stp x29, x30, [sp, #-16]!  ; 预减：先 SP -= 16，再写
```
...
| 架构   | 栈增长方向 | 常用寄存器 | 是否强制对齐                 |
| ------ | ---------- | ---------- | ---------------------------- |
| x86    | 向下       | ESP        | 通常4字节对齐                |
| x86-64 | 向下       | RSP        | 必须16字节对齐（函数调用前） |
| ARM    | 向下       | SP (R13)   | 通常4或8字节对齐             |
| ARM64  | 向下       | SP         | 必须16字节对齐               |


### 各平台堆增长方向

堆（Heap）在编程语言和操作系统中是**动态分配内存区域**。尽管它在高级语言中被自动
管理（如通过 `malloc` / `new` / `free` 等接口），但底层实现依赖于平台、体系架构、
操作系统和运行时库的协同机制。

以下从**x86-32、x86-64、ARM（32位）和 ARM64（64位）**四个平台，分别讲解**堆的管理
机制、增长方向和分配实现原理**。
...
| 特性     | 描述                                                       |
| -------- | ---------------------------------------------------------- |
| 作用     | 管理程序运行时动态分配的内存（如 malloc/new）              |
| 起始地址 | 由操作系统决定，通常位于 BSS 段之后                        |
| 增长方向 | **向上增长**（从低地址向高地址）                           |
| 生命周期 | 手动管理（如 C 的 malloc/free）或自动管理（如 GC、RAII）   |
| 共享行为 | 线程/进程共享或隔离，取决于系统调用（如 brk/sbrk 或 mmap） |
...
* 增长方向
    * 堆从**低地址向高地址增长**
* 分配方式
    * 使用 `brk()` / `sbrk()` 系统调用来调整进程的“程序 break”位置
    * 或使用 `mmap()` 分配匿名内存区域（用于大块或碎片回收）
* 管理逻辑（libc）
    * C 库（如 glibc）内部维护 free list 或 chunk（如 ptmalloc2）
    * 分配时可能使用 first-fit/best-fit 策略

---

x86-64（64位）
* 增长方向
    * 同样是**向上增长**，空间更大（虚拟地址空间扩大）
* 分配方式
    * 小块使用 `brk()` / `sbrk()` 调整堆顶
    * 大块直接使用 `mmap()`（避免影响主堆）
    * glibc、tcmalloc、jemalloc 等运行时库支持多线程/更高性能管理
* 地址空间布局
    * 堆位于 **BSS段之后**，低于共享库、栈、内核空间
    ```text
    高地址
     ┌────────────┐
     │ Stack      │ ← 向下增长
...
* 增长方向
    * 与 x86 一样，堆**向上增长**
* 分配方式
    * 通常使用 `sbrk()` 或 `mmap()`（Linux 下）
    * 运行时库（如 `uClibc`, `newlib`, `glibc`）决定实现
* 嵌入式差异
    * 在裸机或小型嵌入式设备上，没有操作系统提供 `sbrk()`，堆通常为编译时固定的 buffer
...
* 增长方向
    * 同样为**向上增长**
* 分配方式
    * Linux 用户态程序通常也使用 `brk()` + `mmap()`，或纯 `mmap()` 分配匿名区域
    * 在 Android 系统中，堆分配由 `jemalloc` 或 `scudo` 实现
* 多线程支持
    * 多核优化下的堆实现会为每个线程维护线程局部分配区域（tcache）
...
堆分配示意图（典型 ELF 程序结构）
```text
Memory Address Layout (simplified):
...
* 所有主流平台的 **堆增长方向均为向上（低地址 → 高地址）**
* 堆由操作系统提供接口（`brk`, `sbrk`, `mmap`）管理
* 堆分配算法和管理策略依赖运行时库（glibc, jemalloc 等）
* 多线程或嵌入式环境会有不同堆分配机制（tcache、小堆、预分配等）



## 运行库

入口函数/入口点（Entry Point），是指一个程序的初始化和结束部分，它往往是运行库的
一部分。一个典型的程序运行步骤大致如下：
* 操作系统创建进程后，把控制权交给了程序的入口，这个入口往往是运行库中的某个入口
  函数
* 入口函数对运行库和程序运行环境进行初始化，包括堆、I/O、线程、全局变量构造，等等
* 入口函数在完成初始化之后，调用main函数，正式开始执行程序主体部分
* main函数执行完后，返回到入口函数，入口函数进行清理工作，包括全局变量析构、堆销毁、
  关闭I/O等，然后进行系统调用结束进程

文件句柄的意义在于，可以防止用户随意读写操作系统内核的文件对象，文件句柄总是和内核
的文件对象关联的，内核可以通过句柄来计算出内核里文件对象的地址，但此能力不对用户
开放。

在内核中，每一个进程都有一个私有的“打开文件表”，这个表是一个指针数组，每一个元素
都指向一个内核的打开文件对象。而fd，就是这个表的下标。当用户打开一个文件时，内核
会在内部生成一个打开文件对象，并在这个表里找到一个空项，让这一项指向生成的打开文件
对象，并返回这一项的下标作为fd。由于这个表处于内核，并且用户空间无法访问到，因此
用户即便拥有fd，也无法得到打开文件对象的地址，只能够通过系统提供的函数来操作。

对于Windows系统中的fd，与Linux系统大同小异，不过Windows的句柄并不是打开文件表的
下标，而是其下标经过某种线性变换之后的结果。

I/O初始化时，首先需要在用户空间建立stdin、stdout、stderr及其对应的file结构，使得
程序进入main之后可以直接使用pringf、scanf等函数。

运行时库（Runtime Library）至少包括入口函数，及其所依赖的函数构成的函数集合。当然，
他还理应包括各种标准库函数的实现，而C语言的运行库被称为C运行库（CRT）。
...
美国国家标准协会（American National Standards Institute，ANSI）建立的C语言标准
被称为ANSI C。C89建立于1989年，是第一个完整的C语言标准，由C89指定的C语言基础函数库
就称为ANSI C标准运行库（简称标准库）。1995年C语言标准委员会对C89标准进行了一次修订，
ANSI C标准库得到了一次扩充。C99在1999年诞生，C语言标准库得到进一步扩充。

glibc即GNU C Library，是GNU旗下的C标准库。最初由自由软件基金会FSF（Free Software
 Foundation）发起开发，目的是为GNU操作系统开发一个C标准库。GNU最初计划的内核是Hurd，
但由于其发展缓慢，后续就由Linux取代了Hurd成为GNU操作系统的内核。

## 系统调用

每个操作系统都会提供一套接口，这些接口往往通过中断来实现，例如Linux使用0x80号中断
作为系统调用的入口，Windows采用0x2E号中断作为系统调用的入口。

在x86下，各个通用寄存器用户传递参数，EAX寄存器用户表示系统调用的接口号，比如EAX=1
表示退出进程（exit）；EAX=2表示创建进程（fork）；EAX=3表示读取文件或IO（read）；
EAX=4表示写文件或者IO（write）等，每个系统调用都对应于内核源代码中的一个函数，它们
都是以“sys_”开头的，比如exit调用对应内核中的sys_exit函数。当系统调用返回时，EAX
又作为调用结果的返回值。

从用户态切换到内核态一般也是通过中断来实现。系统调用都有一个系统调用号，这个系统
调用号在执行int指令前，会被放置在某个固定的寄存器里，对应的中断代码会取得这个系统
调用号，并且调用正确的函数。以Linux的int 0x80为例，系统调用号是由eax来传入的。用户
将系统调用号放入eax，然后使用int 0x80调用中断，中断服务程序就可以从eax里取得系统
调用号，进而调用对应的函数。

在实际执行中断向量表中的第0x80号元素所对应的函数之前，CPU首先还要进行栈的切换。
在Linux中，用户态和内核态使用的是不同的栈，两者各自负责各自的函数调用，互不干扰。
但在应用程序调用0x80号中断时，程序的执行流从用户态切换到内核态，这时程序的当前栈
必须也相应的从用户栈切换到内核栈。从中断处理函数中返回时，程序的当前栈还要从内核
栈切换回用户栈。

所谓的“当前栈”，指的是ESP的值所在的栈空间。如果ESP的值位于用户栈的范围内，那么程序
的当前栈就是用户栈，反之亦然，另外寄存器SS的值还应该指向当前栈所在的页。当前栈由
用户栈切换为内核栈的实际行为是：
1. 保存当前的ESP、SS的值
2. 将ESP、SS的值设置为内核栈的相应值

反过来，将当前栈由内核栈切换为用户栈的实际行为是：
1. 恢复原来的ESP、SS值

当0x80号中断发生的时候，CPU除了切入内核态之外，还会自动完成下列几件事：
1. 找到当前进程的内核栈（每一个进程都有自己的内核栈）
2. 在内核栈中以此压入用户态的寄存器SS、ESP、EFLAGS、CS、EIP

当内核从系统调用中返回的时候，需要用iret指令来回到用户态，iret指令则会从内核栈里
弹出寄存器SS、ESP、EFLAGS、CS、EIP的值，使得栈恢复到用户态的状态。

由于基于int指令的系统调用在奔腾4代处理器上性能不佳，Linux在2.5版本起开始支持一种
新型的系统调用机制。

执行`ldd /bin/ls`可以看到linux-vdso.so.1没有任何的实际文件与之对应，这个库是Linux
用户支持新型系统调用的“虚拟”共享库。linux-vdso.so.1并不存在实际的文件，他只是操作
系统生成的一个虚拟动态共享库（Virtual Dynamic Shared Library，VDSO）。对于不同的
操作系统，该库的位置可能是不同的，但同一操作系统一般相同。可以通过Linux的proc文件
系统来查看可执行文件的内存映像，可以看到是存在vdso文件的，可以用如下方法将它导出
到一个文件里：
```
$ dd if=/proc/self/mem of=linux_vdso.dso bs=4096 skip=1048574 count=1
```
这里假设vdso的地址为0xffffe000-0xfffff000，1048574=0xFFFFE，mem等价于当前进程的
内存快照。

将vdso反汇编可以看到一个没见过的系统指令：sysenter。这就是intel在奔腾2代处理器开始
